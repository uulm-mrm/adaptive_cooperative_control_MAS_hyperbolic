...
freq2 = 1.5*pi;
% Sr = [0 freq; -freq 0];
% w = quantity.Discrete([sin(tDomain.grid*freq), cos(tDomain.grid*freq)], tDomain);
...
% S = quantity.Piecewise({[0 freq; 0 0] * quantity.Discrete.ones(1, splitDomain(1)), [0 freq2; 0 0] * quantity.Discrete.ones(1, splitDomain(2))});
...
v = quantity.Piecewise({quantity.Discrete([sin(freq*splitDomain(1).grid), cos(freq*splitDomain(1).grid)], splitDomain(1)), ...
    quantity.Discrete([sin(freq2*splitDomain(2).grid), cos(freq2*splitDomain(2).grid)], splitDomain(2))});
% v = quantity.Piecewise({quantity.Discrete([splitDomain(1).grid*freq, ones(size(splitDomain(1).grid))], splitDomain(1)), ...
%     quantity.Discrete([simTime/2*freq + splitDomain(2).grid*freq2, ones(size(splitDomain(2).grid))], splitDomain(2))});
...
% 	d_eff = g1*d;
% 	data.r = export.dd(...
%     	'M', [tDomain.grid, d_eff.valueDiscrete], ...
%     	'header', {'t', 'r1'}, ...
